'''
This is the script for all miscellanous installations
'''

from templates.video_functions import *
from templates.video_emotions import *
...
### Emotions
emotion_df = optimise_video_peopleEmotions_df[optimise_video_peopleEmotions_df['Aspect']=='Emotion']
emotion_df = emotion_df.reset_index(drop=True)
emotion_df['context'] = 'NA'
for index,row in emotion_df.iterrows():
    ...
    while attempt < max_attempts and not success:
        try:
            ...
            emotionContext_elements = elementExtraction_video(context_extraction_response_schema,
                                                              context_extraction_prompt_system,
                                                              context_extraction_prompt, row.Description)
            emotion_df.loc[index, 'context'] = extract_context_analysis_data_to_dataframe(emotionContext_elements)

            success = True  # Set success flag to True if the call succeeds
            print(f"Successfully processed row {index}")

            # Save the consolidated dataframe
            with open('optimise_output/optimise_video_invokedEmotion_df.pkl', 'wb') as f:
